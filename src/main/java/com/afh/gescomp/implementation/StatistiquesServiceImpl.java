...
public class StatistiquesServiceImpl implements StatistiquesService {
    ...
    // Pattern pour valider les paramètres de structure
    private static final Pattern VALID_STRUCT_PATTERN = Pattern.compile("^[0-9]{2}$");
    
    // Générateur de nombres aléatoires sécurisé pour les données par défaut
    ...
    public Map<String, Object> getFournisseursStatistiques(String numStruct, int page, int size,
        ...
        try {
            ...
            sql.append("SELECT ");
            sql.append("f.DESIGNATION, ");
            sql.append("f.NUM_FOURN, ");
            sql.append("COUNT(m.NUM_MARCHE) as nombre_marches, ");
            sql.append("COALESCE(SUM(m.MNT_MARCHE), 0) as montant_total, ");
            sql.append("COALESCE(SUM(mp.MONTANT_PEN), 0) as penalites, ");
            sql.append("b.DESIGNATION as banque ");
            sql.append("FROM ACHAT.FOURNISSEUR f ");
            sql.append("LEFT JOIN ACHAT.MARCHE m ON f.NUM_FOURN = m.NUM_FOURN ");
            sql.append("LEFT JOIN ACHAT.BANQUE b ON m.NUM_BANQUE = b.NUM_BANQUE ");
            sql.append("LEFT JOIN ACHAT.MRC_PENALITE mp ON m.NUM_MARCHE = mp.NUM_MARCHE ");
            sql.append("WHERE 1=1 ");

            Query query = entityManager.createNativeQuery(sql.toString());

            // Ajout sécurisé des conditions avec paramètres liés
            if (isValidStruct(numStruct)) {
            ...
            if (isValidFilter(filterName)) {
                sql.append("AND UPPER(f.DESIGNATION) LIKE UPPER(:filterName) ");
                query = entityManager.createNativeQuery(sql.toString());
                if (isValidStruct(numStruct)) {
                    query.setParameter("numStruct", numStruct);
                }
            ...
            sql.append("GROUP BY f.DESIGNATION, f.NUM_FOURN ");

            if (filterMinAmount != null && filterMinAmount > 0) {
                sql.append("HAVING COALESCE(SUM(m.MNT_MARCHE), 0) >= :filterMinAmount ");
                query = entityManager.createNativeQuery(sql.toString());
                if (isValidStruct(numStruct)) {
                    query.setParameter("numStruct", numStruct);
                }
            ...
            if (filterHasPenalites != null && filterHasPenalites) {
                sql.append((filterMinAmount != null ? "AND " : "HAVING ") + "COALESCE(SUM(mp.MONTANT_PEN), 0) > 0 ");
                query = entityManager.createNativeQuery(sql.toString());
                if (isValidStruct(numStruct)) {
                    query.setParameter("numStruct", numStruct);
                }
            ...
            sql.append("ORDER BY nombre_marches DESC, montant_total DESC ");
            sql.append("OFFSET :offset ROWS FETCH NEXT :size ROWS ONLY");

            query = entityManager.createNativeQuery(sql.toString());
            
            // Réapplication sécurisée de tous les paramètres
            if (isValidStruct(numStruct)) {
                query.setParameter("numStruct", numStruct);
            }
            if (isValidFilter(filterName)) {
    ...
    public Map<String, Object> getRegionsRepartitionByDates(String numStruct, String dateDebut, String dateFin) {
        ...
        try {
            ...
            for (int i = 0; i < defaultLabels.size(); i++) {
                defaultData.add(generateSecureRandomDefaultData());
            }
    ...
    public Map<String, Object> getFournisseursAvecMarches(String numStruct, int page, int size, String filterName, Double filterMinAmount) {
        ...
        try {
            ...
            logger.error("Erreur dans getFournisseursAvecMarches: {}", e.getMessage(), e);
    ...
    public Map<String, Object> getFournisseurComplet(String numFourn) {
        ...
        try {
            ...
            result.put("error", "Fournisseur non trouvé: " + numFourn);
        } catch (Exception e) {
            result.put("status", "error");
            result.put("error", "Erreur lors de la récupération du fournisseur complet: " + e.getMessage());
            logger.error("Erreur lors de la récupération du fournisseur complet: {}", e.getMessage(), e);
        }
        return result;
    }
    ...
    public Map<String, Object> getMarchesEvolutionByPeriod(String numStruct, String period) {
        ...
        try {
            ...
            StringBuilder sql = new StringBuilder();
            sql.append("SELECT ");
            sql.append("TO_CHAR(m.DATE_MARCHE, 'YYYY-MM') as mois, ");
            sql.append("COUNT(*) as nombre_marches ");
            sql.append("FROM ACHAT.MARCHE m ");
            sql.append("WHERE m.DATE_MARCHE >= ADD_MONTHS(SYSDATE, -:months) ");
            ...
            sql.append("GROUP BY TO_CHAR(m.DATE_MARCHE, 'YYYY-MM') ");
            sql.append("ORDER BY TO_CHAR(m.DATE_MARCHE, 'YYYY-MM')");
            ...
            if (isValidStruct(numStruct)) {
                sqlActive += " AND (:numStruct = '03' OR m.NUM_STRUCT = :numStruct)";
            }
            ...
            if (isValidStruct(numStruct)) {
                sqlDelay += " AND (:numStruct = '03' OR m.NUM_STRUCT = :numStruct)";
            }
    ...
    public Map<String, Object> getMarchesEvolutionByDates(String numStruct, String dateDebut, String dateFin) {
        ...
        try {
            ...
            sql.append("COUNT(*) as nombre_marches ");
            sql.append("FROM ACHAT.MARCHE m ");
            sql.append("WHERE m.DATE_MARCHE >= TO_DATE(:dateDebut, 'YYYY-MM-DD') ");
            ...
            // Métriques pour la période donnée
    ...
    private List<Integer> getDefaultEvolutionData(String period) {
        ...
        for (int i = 0; i < months; i++) {
            data.add(generateSecureRandomDefaultData());
        }
    ...
    public byte[] exportToExcel(String numStruct, String type, String dateDebut, String dateFin) {
        try {
            ...
            logger.error("Erreur lors de la génération du fichier Excel: {}", e.getMessage(), e);
    ...
    private class FooterPageEvent extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            try {
                ...
                logger.error("Erreur lors de l'ajout du pied de page au document: {}", e.getMessage(), e);
            }
        }
    }
    ...
    private List<Map<String, Object>> getMarchesDataByPeriod(String numStruct, String dateDebut, String dateFin) {
        try {
            ...
            logger.error("Erreur lors de la récupération des données de marchés: {}", e.getMessage(), e);
            return new ArrayList<>();
        }
    }
    ...
    private void exportFournisseursStatistiques(Document document, String numStruct, String dateDebut, String dateFin) throws Exception {
        ...
        for (Map<String, Object> fournisseur : fournisseurs) {
            ...
            Font fournisseurNumeroFont = arabicFontUtil.getAppropriateFont(numero, 9, Font.NORMAL);
            PdfPCell numeroCell = new PdfPCell(new Phrase(numero, fournisseurNumeroFont));
            ...
            Font fournisseurNombreMarchesFont = arabicFontUtil.getAppropriateFont(nombreMarches, 9, Font.NORMAL);
            PdfPCell marchesCell = new PdfPCell(new Phrase(nombreMarches, fournisseurNombreMarchesFont));
            marchesCell.setPadding(5);
            marchesCell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table.addCell(marchesCell);
    ...
    private void exportMarchesStatistiques(Document document, String numStruct, String dateDebut, String dateFin) throws Exception {
        ...
        for (Map<String, Object> marche : marches) {
            ...
            Font marcheStdFont = arabicFontUtil.getAppropriateFont("", 9, Font.NORMAL); // Police standard pour le français
            ...
            // Pour le fournisseur
            PdfPCell fournisseurCell = new PdfPCell(new Phrase(fournisseurFormatted, marcheFournisseurFont));
            fournisseurCell.setPadding(5);
    ...
    public List<String> generateMonthLabels(String dateDebut, int nombreMois) {
        ...
        try {
            ...
            logger.error("Erreur lors de la récupération des étiquettes de mois par défaut: {}", e.getMessage(), e);
    ...
    public Map<String, Object> getArticlesBySecteur(String numStruct) {
        ...
        try {
            ...
            logger.error("Erreur lors de la récupération des articles par secteur: {}", e.getMessage(), e);
    ...
    public Map<String, Object> getDecomptesByType(String numStruct) {
        ...
        try {
            ...
            logger.error("Erreur lors de la récupération des décomptes par type: {}", e.getMessage(), e);
    ...
    public Map<String, Object> getStatistiquesFournisseursComplet(String numStruct) {
        ...
        try {
            ...
            logger.error("Erreur lors de la récupération des statistiques des fournisseurs avec pénalités: {}", e.getMessage(), e);
        }
    ...
    public Map<String, Object> getStatistiquesGenerales(String numStruct) {
        ...
        try {
            ...
            logger.error("Erreur lors de la récupération des statistiques générales: {}", e.getMessage(), e);
        }
    ...
    private boolean isValidStruct(String numStruct) {
        return numStruct != null && !numStruct.trim().isEmpty() && VALID_STRUCT_PATTERN.matcher(numStruct).matches();
    }

    private boolean isValidFilter(String filter) {
        return filter != null && !filter.trim().isEmpty() && filter.length() <= 100;
    }
    ...
    /**
     * Génère un nombre aléatoire sécurisé entre 1 et 10 pour les données par défaut
     * @return un nombre aléatoire entre 1 et 10
     */
    private static int generateSecureRandomDefaultData() {
