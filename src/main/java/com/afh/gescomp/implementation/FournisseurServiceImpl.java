    ...
    public byte[] generateFournisseurDetailsPDF(String numFourn) {
        try {
            ...
            try {
                ...
                logger.warn("Logo non trouvé: {}", e.getMessage(), e);
            }
            ...
            logger.error("Erreur lors de la génération du PDF pour le fournisseur {}: {}", numFourn, e.getMessage(), e);
...
/*        if((fournisseurDesignation == null || fournisseurDesignation.isEmpty()) || (designation == null || designation.isEmpty())){
    ...
    public void resetSequence() {
        ...
        // Validation sécurisée de la valeur startValue
        if (startValue != null && startValue > 0 && startValue <= Long.MAX_VALUE) {
            // Utilisation de PreparedStatement pour éviter l'injection SQL
            String alterSql = "ALTER SEQUENCE FOURNISSEUR_SEQ RESTART START WITH ?";
            jdbcTemplate.update(alterSql, startValue);
        } else {
            // Valeur par défaut sécurisée
            String alterSql = "ALTER SEQUENCE FOURNISSEUR_SEQ RESTART START WITH ?";
            jdbcTemplate.update(alterSql, 1L);
        }
    }
